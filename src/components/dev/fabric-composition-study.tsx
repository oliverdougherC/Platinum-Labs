...
type StudyViewMode = "activity" | "relationship-map";
...
function relationshipPresentation(relationship: FabricRelationship): RelationshipPresentation {
  const state = resolveStudyRelationshipState(relationship);
  const width = Number(relationship.width.toFixed(2));
  const animated = state === "live-transfer" && relationship.animated;
...
export function FabricCompositionStudy({
  ...
  const relationshipPresentationById = useMemo(
    () => new Map(model.relationships.map((relationship) => [relationship.id, relationshipPresentation(relationship)])),
    [model.relationships],
  );
    ...
    >
      <section data-study-stage-frame className="fabric-study-stage-frame" aria-label={scene.title}>
        <div className="fabric-study-heading">
          <div>
            <p>{mapMode ? "Declared relationship map" : "V3 composition study"}</p>
            <h1>{scene.title}</h1>
          </div>
